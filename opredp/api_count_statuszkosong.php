...
try {
    // Query SQL untuk menghitung status Z yang kosong di bulan berjalan
    $sql = "
        SELECT 
            COUNT(*) AS total
        FROM tbtr_mstran_d
        WHERE
            to_char(mstd_create_dt, 'YYYYMM') = to_char(current_date, 'YYYYMM')
            AND mstd_typetrn = 'Z'
            AND mstd_flagdisc1 IS NULL
            AND mstd_flagdisc2 IS NULL;
    ";
